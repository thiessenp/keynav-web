...
// TODO: ESLint insists the below class should have a return - odd...
/**
 * Basically creating a custom SELECT element out of random elements
 *
 * NOTE: Simpler version of ./list.js, main purpose is to be easier to
 * understand or copy and paste into code / similar (isolated code).
 */
export class ListNav {
    ...
    /**
     * Does something when the user keys a valid key, like going to the next item.
     * @param {Event} e - event fired on element
    ...
    handleKey(e, key) {
      ...
      switch (key) {
        ...
        case 'ArrowDown':
        case 'ArrowRight': {
          // Stop browser moving on a directional key
          e.preventDefault();
    ...
    /**
     * @return {Element} last item in the list of nav items.
     */
    getLast() {
      return this.items()[this.items().length - 1];
    }
    ...
    /**
     * @return {Element} first item in the list of nav items.
     */
    getFirst() {
      return this.items()[0];
    }
    ...
    /**
     * @return {Element} currently active element in the list of nav items.
     */
    getActive() {
