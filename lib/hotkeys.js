    ...
    addBehavior() {
        ...
        this.sub({
            ...
            callback: (e) => {
                ...
                if (this.isInputElement(e) && !isInputException) { return; }

                if (!this.isValidHotkey(key)) { return; }
    
                // Parse the key to a string
                let combinedKey = '';
                if (modifier === 'Alt') { combinedKey = 'alt+'; }
                if (modifier === 'Ctrl') { combinedKey = 'ctrl+'; }
                if (modifier === 'Shift') { combinedKey = 'shift+'; }
                combinedKey += key.toLowerCase();
    
                // Use the key string to get the related key object
                var keyObj = this.hotkeys[combinedKey];
                if (!keyObj) return;
